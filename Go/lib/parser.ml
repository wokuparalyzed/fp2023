...
let str_const =
  ...
  *> take_till (function
    | '\"' -> true
    | _ -> false)
  <* char '\"' >>= fun res -> return (String res);;
...
let identifier_parser =
  ...
  let is_ident_mid = function
    ...
    | _ -> false
  in
  satisfy is_ident_start >>= fun first -> take_while is_ident_mid >>= fun rest -> return (string_of_char first ^ rest)
;; 
...
let spend_char c =
  option ' ' (char c) >>= fun _ -> return() 
;;

let comma_separated p =
  (sep_by (char ',' <* spaces) (p <* spaces)) >>= fun ps -> spend_char ',' >>= fun _ -> return ps
;;

(* ***********************************TYPES*********************************** *)

let type_name_parse =
  identifier_parser >>= function
...
let arg_decl = (identifier_parser <* space1) >>= fun name -> type_name_parse >>= fun typ -> return (name, typ);;

let parse_args = parens (comma_separated arg_decl)

let signature = parse_args >>= fun args -> parse_option_res >>= fun ret -> return { args; ret };;
...
let ex_decl_disp =
  ...
  let parse_anon_func d = keyword "func" *> spaces *> signature >>= fun signat -> spaces *> d.block d >>= fun block -> return (AnonFunc(signat, block)) in
  ...
  let expression d =
    fix (fun expression ->
        ...
        ident
        >>= fun n -> match n with
        | (Identifier "Print") -> (parens (comma_separated expression) >>= fun a -> return(PrintCall a))
        | (Identifier "len") -> (parens (parse_const_string) >>= fun str -> return(LenCall str))
        | _ -> (parens (comma_separated expression) >>= fun a -> return (FunCall (n, a))) in
      let expr = parse_const_string <|> parse_const_int <|> parse_fun_call <|> parse_anon_func d <|> bool_const <|> ident <|> nil_const <|> parens expression in
  ...
  in
  let global_func_declaration d = fix @@ fun _ -> func_declaration d >>= fun r -> return (FuncTopLevelDeclaration r)
  in
  let var_declaration d =
    keyword "var" *> space1 *> identifier_parser >>= fun var_name -> spaces *> parse_ident >>= fun ident -> spaces *> (char '=' *> spaces *> expression d <* semi >>= fun val_ -> return (var_name, ident, val_)) <|> (semi *> expression d >>= fun nil -> return(var_name, ident, nil)) in
  let global_var_declaration d = fix @@ fun _ -> var_declaration d >>= fun r -> return (VarTopLevelDeclaration r) in
  let global_declaration d = (fix @@ fun _ -> global_func_declaration d <|> global_var_declaration d) in
  (* ***********************************STATEMENTS*********************************** *)
  let ret_stmt d =
    let ret_void =
      keyword "return" *> spaces *> semi *> return (ReturnStatement None)
    in
    let ret_expr =
      spaces *> keyword "return" *> space1 *> expression d <* spaces <* semi >>= fun expr -> return(ReturnStatement (Some expr))
    in
    ret_expr <|> ret_void <?> "ReturnStatement"
  in
  let block d = braces (many (spaces *> d.statements d <* spaces)) in
  let block_stmt d =
    block d >>= fun bl -> return (BlockStatement bl)
  in
  let if_stmt d = keyword "if" *> space1 *> expression d <* spaces >>= fun cond -> block d <* spaces >>= fun then_ -> peek_char_fail >>= fun char_ ->
      if char_ == 'e' then keyword "else" *> spaces *> block d >>= fun else_ -> return(IfElseStatement (cond, then_, else_)) else return(IfElseStatement (cond, then_, []))
  in
  let for_stmt d = keyword "for" *> space1 *> expression d <* spaces >>= fun cond -> block d <* spaces >>= fun bl -> return(ForStatement(cond, bl)) in
  let expr_stmt d = d.expression d >>= fun expr -> spaces *> semi *> return (ExpressinStatement(expr)) in
  let assign_stmt d = d.expression d <* spaces >>= fun l -> char '=' *> spaces *> d.expression d <* spaces <* semi >>= fun r -> return(AssignStatement(l, r)) in
  ...
  let statements d = fix @@ fun _ -> ret_stmt d <|> block_stmt d <|> expr_stmt d <|> var_stmt d <|> assign_stmt d <|> for_stmt d <|> if_stmt d <|> break_stmt <|> continue_stmt in
...
let rec_top_level : file t =
  let global_decl_list =
    fix @@ fun top_level_decl_list ->
      at_end_of_input >>= fun finished ->
    if finished
    then return []
    else
      global_declaration <* spaces >>= fun d -> top_level_decl_list >>= fun tail ->
      return (d :: tail)
  in
  global_decl_list
;;
      ...
      ; ReturnStatement
          (Some
             (BinaryOp
                ...
                , Asterisk
                , FunCall
                    (Identifier "f", [ BinaryOp (Identifier "x", MinusBin, Const (Integer 1)) ])
                )))
      ] )
  = parse1
      func_decl
...
Ok
 ("fact",
  ...
  [IfElseStatement
    (BinaryOp (Identifier "x", Equal,
      Const (Integer 0)),
    [ReturnStatement
      (Some (Const (Integer 1)))],
    [ReturnStatement
      (Some
        (UnaryOp
          (MinusUn, Const (Integer 1))))]);
   ReturnStatement
    (Some
      (BinaryOp (Identifier "x", Asterisk,
        FunCall (Identifier "f",
         [BinaryOp (Identifier "x", MinusBin,
           Const (Integer 1))]))))])
 ...
 ("fact",
  ...
  [IfElseStatement
    (BinaryOp (Identifier "x", Equal,
      Const (Integer 0)),
    [ForStatement
      (BinaryOp (Identifier "a", Less,
        Identifier "b"),
      [AssignStatement (Identifier "a",
        BinaryOp (Identifier "a", Plus,
         Const (Integer 1)))]);
     ReturnStatement
      (Some (Const (Integer 1)))],
    []);
